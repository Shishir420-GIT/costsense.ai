...
#  You can have Nested cluster as Shown below
#  Valid Arrow types: solid, dashed, double-dotted, double
#  label : Optional can be any name or description 
#          for the connection
#  Right Click on the Node from the icons list to copy the 
#        Node Name and Use name property for the customized name

################################################

Cluster: VPC
  Cluster: Subnet
    Node: EC2 [name=EC2 Server]
Cluster: Storage
  Node: SimpleStorageService [name=S3]
EC2 Server -> S3 [arrow=solid, label=Role]`;

const initialClusters: Cluster[] = [
  { id: 'vpc', label: 'VPC' },
  { id: 'subnet', label: 'Subnet', parentId: 'vpc' },
  { id: 'storage', label: 'Storage' },
];
const initialNodes: Node[] = [
  { id: 'ec2', label: 'EC2', name: 'EC2 Server', x: 220, y: 140, color: '#e3f2fd', clusterId: 'subnet' },
  { id: 's3', label: 'SimpleStorageService', name: 'S3', x: 420, y: 220, color: '#ffe0b2', clusterId: 'storage' },
];
const initialEdges: Edge[] = [
  { id: 'e1', from: 'ec2', to: 's3', label: 'Role', type: 'solid' },
];
...
function App() {
  ...
  // Make canvas size dynamic based on content
  ...
  // Resize canvas to fit content with reasonable bounds
  useEffect(() => {
    const padding = 120;
    ...
    setCanvasSize(cs => {
      // Set size to fit content with minimum bounds, allow shrinking
      const newW = Math.max(neededW, 800);
      const newH = Math.max(neededH, 600);
  ...
  const fitToContent = () => {
    ...
    setZoom(clamped);
    // Center content by scrolling to make it visible
    setTimeout(() => {
      if (!svgWrapperRef.current) return;
      // Calculate content bounds center
      const contentCenterX = (bbox.minX + bbox.maxX) / 2;
      const contentCenterY = (bbox.minY + bbox.maxY) / 2;
      
      // Simple centering: scroll to show content center
      // At zoom=1, SVG coordinates map directly to pixels
      const wrapperWidth = svgWrapperRef.current.clientWidth;
      const wrapperHeight = svgWrapperRef.current.clientHeight;
      
      const scrollX = Math.max(0, contentCenterX - wrapperWidth / 2);
      const scrollY = Math.max(0, contentCenterY - wrapperHeight / 2);
      
      svgWrapperRef.current.scrollLeft = scrollX;
      svgWrapperRef.current.scrollTop = scrollY;
    }, 50);
  };
  ...
  const findClusterAt = (x: number, y: number): string | undefined => {
    // Prefer the most-specific (deepest) child cluster when clusters are nested/overlapping.
    // Build a list of candidate clusters whose bbox contains the point, then pick the one
    // with greatest depth (number of ancestors). Tie-breaker: smallest area (more specific).
    const candidates: { id: string; depth: number; area: number }[] = [];
    const depthCache = new Map<string, number>();
    const getDepth = (cid: string | undefined): number => {
      if (!cid) return 0;
      if (depthCache.has(cid)) return depthCache.get(cid)!;
      let d = 0;
      let cur = clusters.find(c => c.id === cid);
      while (cur && cur.parentId) {
        d++;
        cur = clusters.find(cc => cc.id === cur!.parentId);
      }
      depthCache.set(cid!, d);
      return d;
    };
    for (const cluster of clusters) {
      ...
      if (bbox && x >= bbox.x && x <= bbox.x + bbox.w && y >= bbox.y && y <= bbox.y + bbox.h) {
        ...
        const depth = getDepth(cluster.id);
        candidates.push({ id: cluster.id, depth, area });
      }
    }
  ...
  const finalizeNodePosition = (nodeId: string) => {
    const node = nodes.find(n => n.id === nodeId);
    if (!node) return;
    // Prefer the deepest child cluster at the node position
    const cid = findClusterAt(node.x, node.y);
    setNodes((prev: Node[]) => {
      let updated = prev.map((n: Node) => n);
      // If node is dropped inside a cluster, check for overlap and snap to a free nearby position
      if (cid) {
        // find other members of that cluster
        const members = prev.filter(n => n.clusterId === cid && n.id !== nodeId);
        // simple overlap check: if any member is within 72px box of current node, snap
        const overlapping = members.some(m => Math.abs(m.x - node.x) < 72 && Math.abs(m.y - node.y) < 72);
        if (overlapping) {
          const pos = findPositionNearCluster(cid, 180, 100);
          updated = prev.map((n: Node) => n.id === nodeId ? { ...n, clusterId: cid, x: pos.x, y: pos.y } : n);
        } else {
          updated = prev.map((n: Node) => n.id === nodeId ? { ...n, clusterId: cid, x: node.x, y: node.y } : n);
        }
      } else {
        // not inside any cluster: clear clusterId
        updated = prev.map((n: Node) => n.id === nodeId ? { ...n, clusterId: undefined, x: node.x, y: node.y } : n);
      }
  ...
  const handleParsePrompt = async (promptText?: string) => {
    ...
    try {
      ...
      // pass preservation hints so existing matched nodes/clusters keep their positions
      const layoutLocal = layoutClusters(mergedLocal.clusters, mergedLocal.nodes, undefined, { preserveNodeIds: mergedLocal.matchedNodeIds, preserveClusterIds: mergedLocal.matchedClusterIds });
    ...
    try {
  const res = await fetch('http://localhost:5001/api/backend/parse', {
        method: 'POST',
  ...
  // compute layout but preserve positions for any matched items so we don't move user-placed nodes/clusters
  const layoutBackend = layoutClusters(mergedBackend.clusters, mergedBackend.nodes, undefined, { preserveNodeIds: mergedBackend.matchedNodeIds, preserveClusterIds: mergedBackend.matchedClusterIds });
  ...
  // mergedNodes contains matched (and newly created) parsed nodes only.
    ...
    // Build matched cluster id set for preservation hints (existing clusters that matched by label)
    const matchedClusterIds = new Set<string>();
    for (const pc of parsed.clusters) {
      const found = existingClusterByLabel.get(pc.label.toLowerCase());
      if (found) matchedClusterIds.add(found.id);
    }

    return { nodes: mergedNodes, edges: mergedEdges, clusters: mergedClusters, matchedNodeIds, matchedClusterIds, nodeMap };
  };
  ...
  const layoutClusters = (clustersToLayout: Cluster[], nodesToLayout: Node[], fixed?: { id: string; x: number; y: number }, opts?: { preserveNodeIds?: Set<string>; preserveClusterIds?: Set<string> }) => {
  ...
  const originY = Math.max(20, Math.floor((canvasSize.h - totalH) / 2 + 20));
    ...
    for (let i = 0; i < assign.length; i++) {
      ...
      // If this cluster id is preserved, keep its existing x/y (if present), otherwise assign new
      const preserveCluster = opts?.preserveClusterIds?.has(clusterObj.id);
      finalClusters.push({ ...clusterObj, x: preserveCluster && clusterObj.x !== undefined ? clusterObj.x : cell.x, y: preserveCluster && clusterObj.y !== undefined ? clusterObj.y : cell.y });
      // shift member nodes
      for (let j = 0; j < finalNodes.length; j++) {
        if (finalNodes[j].clusterId === cid) {
          // If node is preserved, keep its x/y. Otherwise, shift into cluster cell
          const preserveNode = opts?.preserveNodeIds?.has(finalNodes[j].id);
          finalNodes[j] = preserveNode ? { ...finalNodes[j] } : { ...finalNodes[j], x: finalNodes[j].x + dx, y: finalNodes[j].y + dy };
        }
      }
    }
  ...
  const spacingPack = 84; // distance between icons when packed (icon 36 + margin)
    ...
    for (const [cid, idxs] of nodeIndexMap.entries()) {
      ...
      // Respect preserved node ids: do not reposition nodes the caller marked as preserved
      const preservedSet = opts?.preserveNodeIds || new Set<string>();
      const movableIdxs = idxs.filter(i => !preservedSet.has(finalNodes[i].id));
      if (movableIdxs.length === 0) continue; // nothing to move
  ...
  const startY = center.cy - ((rowsPack - 1) * spacingPack) / 2;
      for (let k = 0; k < movableIdxs.length; k++) {
...
const allIcons = (iconCategories.flatMap(cat => cat.icons) as IconDef[]).concat(uploadedIcons as IconDef[]);
  ...
  const handleSaveSVG = async () => {
    // Use the ref to get the diagram SVG
    ...
    // Check if there are any nodes or clusters to export
    if (nodes.length === 0 && clusters.length === 0) {
      console.error('No diagram content to export - add some nodes or clusters first');
      return;
    }
    
    // Wait a small amount of time to ensure SVG is fully rendered
    await new Promise(resolve => setTimeout(resolve, 100));
    
    // Check if there's any content in the SVG
    if (!svg.innerHTML.trim()) {
      console.error('SVG is empty - no diagram content found');
      return;
    }
    ...
    if (bbox.w <= 0 || bbox.h <= 0) {
      console.error('Invalid bounding box - diagram may be empty');
      return;
    }
    ...
    try {
      ...
      // Handle foreignObject elements (emoji icons) by converting them to text elements
      const foreignObjects = Array.from(clone.querySelectorAll('foreignObject'));
      for (const fobj of foreignObjects) {
        try {
          const div = fobj.querySelector('div');
          if (div) {
            const emoji = div.textContent || '';
            const x = parseFloat(fobj.getAttribute('x') || '0');
            const y = parseFloat(fobj.getAttribute('y') || '0');
            const w = parseFloat(fobj.getAttribute('width') || '36');
            const h = parseFloat(fobj.getAttribute('height') || '36');
            const tx = x + w / 2;
      ...
      <header style={{ height: 68, background: 'linear-gradient(90deg,#0f172a,#111827)', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 18px', boxShadow: '0 2px 8px rgba(2,6,23,0.6)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <AIProviderToggle 
            ...
            disabled={aiOpen}
          />
          <AIChatIcon onOpen={() => setAiOpen(true)} />
  ...
  <AIChatWindow 
    ...
    provider={aiProvider}
    onDSLGenerated={async (dsl, explanation) => {
      setPrompt(dsl);
      console.log('AI generated DSL:', { dsl, explanation });
      // Parse the DSL and update the diagram
      await handleParsePrompt(dsl);
      // Center the diagram in viewport after a short delay to ensure DOM updates
      setTimeout(() => {
        fitToContent();
      }, 100);
    }}
      ...
      <section className="prompt-editor">
        ...
        <div className="control-row">
          ...
          inputEl.value = '';
            ...
            <a href={`${import.meta.env.BASE_URL}VizCode-template.xlsx`} download className="btn-help-small" style={{ textDecoration: 'none', display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }}>Template</a>
          </div>
        </div>
        <div className="control-row">
  ...
  <main className="diagram-canvas" style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ marginBottom: 12, display: 'flex', gap: 10 }}>
          ...
          <div className="svg-wrapper"
            ...
            // make the preview area fill available space responsively
            style={{ 
              width: '100%', 
              height: 'calc(100vh - 200px)', // Use available viewport height minus header/controls
              overflowX: 'auto', 
              overflowY: 'auto', 
